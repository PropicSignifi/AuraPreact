describe("$Parser(JSep)", function() {
    const $Parser = window.$Parser;
    const parser = $Parser.getParser('jsep');
    ...
    it("should parse values", function() {
        ...
        const result1 = parser.parse("name", cmp)[0];
        expect(result1(cmp, fn)).toEqual("Wilson");

        cmp.set("v.name", "Test");
        const result2 = parser.parse("v.name", cmp)[0];
        expect(result2(cmp, fn)).toEqual("Test");
    });
